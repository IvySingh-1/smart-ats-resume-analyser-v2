package com.ivy.ats.resumeanalyser.service;

import com.ivy.ats.resumeanalyser.model.AnalysisReport;
import com.ivy.ats.resumeanalyser.model.User;
import com.ivy.ats.resumeanalyser.repository.AnalysisReportRepository;
import com.ivy.ats.resumeanalyser.repository.UserRepository;
...
public class AnalysisService {
    ...
    public AnalysisResult analyzeAndSave(String resumeText, String jdText, String companyType, MultipartFile file,
            String username) {
    ...
    private List<String> runComplianceChecks(String resumeText, MultipartFile file) {
        ...
        if (filename != null && !filename.endsWith(".pdf") && !filename.endsWith(".docx")) {
            warnings.add("File Type: You uploaded a '" + filename.substring(filename.lastIndexOf(".") + 1)
                    + "' file. For ATS, always use .pdf or .docx.");
        }

        // 2. Consistent Dates Check
        Pattern pattern = Pattern.compile(
                "\\b(0[1-9]|1[0-2])\\/(\\d{4})\\b|\\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\\s(\\d{4})\\b",
        ...
        if (dateFormatsFound.size() > 1) {
            warnings.add(
                    "Date Format: You use inconsistent date formats (e.g., both 'MM/YYYY' and 'Month YYYY'). Pick one and stick to it.");
        }
    ...
    private List<String> checkDocxCompliance(MultipartFile file) {
        ...
        try (InputStream is = file.getInputStream();
            ...
            if (document.getAllPictures() != null && !document.getAllPictures().isEmpty()) {
                warnings.add(
                        "Formatting: Your .docx file contains images. ATS systems cannot read images; this may cause information to be lost.");
            }

            // Check for tables
            List<XWPFTable> tables = document.getTables();
            if (tables != null && !tables.isEmpty()) {
                warnings.add("Formatting: Your .docx file contains " + tables.size()
                        + " table(s). Complex layouts with tables can confuse an ATS.");
            }
